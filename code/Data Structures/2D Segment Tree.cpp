struct Segtree2D {
    struct Segtree {
        ...
        Segtree(int l, int r) { root = new node(l, r); }
    ...
    Segtree2D(int l, int r) { root = new node(l, r); }

    void update(pnode &nw, int x, int y, int val) {
        int &l = nw->l, &r = nw->r, mid = (l+r)/2;
        if(l == r) nw->y.update(y, val);
        else {
            if(x <= mid) {
                if(!nw->lc) nw->lc = new node(l, mid);
                update(nw->lc, x, y, val);
            } else {
                if(!nw->rc) nw->rc = new node(mid+1, r);
    ...
    int query(pnode &nw, int x1, int x2, int y1, int y2) {
        ...
        int &l = nw->l, &r = nw->r;
        if(r < x1 || x2 < l) return INF;
        if(x1 <= l && r <= x2) return nw->y.query(y1, y2);
